...
export const BoxTypesToolbar: FC<ToolbarProps> = ({ onSearch }) => {
  ...
  return (
    <div className="card-header px-5 py-5 border-b-0 flex-wrap gap-2">
      <h3 className="card-title">{formatMessage({ id: 'SYSTEM.BOX_TYPES' })}</h3>
      <div className="flex flex-wrap items-center gap-2.5">
        {canManageGlobalSettings && (
          <button className="btn btn-sm btn-primary" onClick={handleOpen}>
            {formatMessage({ id: 'SYSTEM.NEW_BOX_TYPE' })}
          </button>
        )}
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder={formatMessage({ id: 'SYSTEM.SEARCH_BOX_TYPE' })}
      ...
      <BoxTypeModal open={modalOpen} onOpenChange={handleClose} />
    </div>
  );
};
