...
export const SubdivisionsViewModal: FC<Props> = ({ open, id, handleClose, handleFormClick }) => {
      ...
      <DialogContent className="container-fixed max-w-screen-md p-0 [&>button]:hidden">
        <DialogHeader className="modal-rounded-t p-0 border-0 relative min-h-20 flex flex-col items-stretch justify-end bg-center bg-cover bg-no-repeat modal-bg">
          <DialogTitle className="absolute top-0 text-1.5xl ml-4 mt-3">
            {formatMessage({ id: 'SYSTEM.SUBDIVISION_DETAILS' })}
          </DialogTitle>
          <DialogDescription />
          <button
              ...
              <div className="card-body grid gap-5">
                <div className="border-b pb-4">
                  <h4 className="text-lg font-semibold mb-4">
                    {formatMessage({ id: 'SYSTEM.SUBDIVISION_INFO' })}
                  </h4>
                  <div className="grid gap-3">
                    <div className="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                      <label className="form-label max-w-56 tex">
                        {formatMessage({ id: 'SYSTEM.SUBDIVISION' })}
                      </label>
